...
**Tasks:**
- [ ] Define a standard `Notifier` interface: `Send(alert core.Alert) error`.
- [ ] Implement the **Webhook Channel** (HTTP POSTs to generic SIEMs).
- [ ] Implement the **Slack Channel** (formatted Slack blocks).
- [ ] Implement the **Email Channel** (SMTP).
- [ ] Wire these to trigger when the Risk Engine hits a `HIGH` or `CRITICAL` threshold.
...
**Tasks:**
- [ ] Set up an HTTP router (Go 1.22+ `net/http` multiplexer or `chi`).
- [ ] Create REST endpoints for historical data:
  - `GET /api/v1/simulations`
  - `GET /api/v1/alerts`
  - `GET /api/v1/events`
- [ ] Hook these into an intermediate in-memory map/slice struct to get the handlers working immediately.
